...
resource "azurerm_monitor_metric_alert" "appinsights" {
  ...
  resource_group_name = azurerm_resource_group.rg.name
  scopes              = [azurerm_application_insights_web_test.appinsights-2[count.index].id,azurerm_application_insights.appinsights.id]
  description         = "Action will be triggered when failed locations exceeds 2"

  application_insights_web_test_location_availability_criteria {
    web_test_id = azurerm_application_insights_web_test.appinsights-2.id
